...
temporary_root="$(mktemp -d /tmp/stado-guidance-check.XXXXXX)"
trap 'rm -rf -- "$temporary_root"' EXIT

GUIDANCE_BUILD_DIR="$temporary_root/first" ./build.sh
GUIDANCE_BUILD_DIR="$temporary_root/second" ./build.sh
...
echo "guidance unit, race, vet, and reproducible unsigned build checks passed"
