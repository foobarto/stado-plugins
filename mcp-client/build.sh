...
if [[ ! -f mcp-client-demo.seed ]]; then
  echo "mcp-client-demo.seed not found. Generate it with:" >&2
  echo "  $STADO plugin gen-key mcp-client-demo.seed" >&2
  exit 1
fi
...
"$STADO" plugin sign plugin.manifest.json --key mcp-client-demo.seed
