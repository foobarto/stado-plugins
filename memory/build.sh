...
output_dir="${MEMORY_BUILD_DIR:-dist}"
mkdir -p "$output_dir"
if [[ -e "$output_dir/plugin.manifest.sig" ]]; then
  echo "refusing to rebuild beside an existing signature; archive the signed release first" >&2
  exit 1
fi

GOCACHE="${GOCACHE:-/tmp/stado-plugins-go-cache}" \
  GOOS=wasip1 GOARCH=wasm \
  go build -trimpath -buildvcs=false -buildmode=c-shared -o "$output_dir/plugin.wasm" .
