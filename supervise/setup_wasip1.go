...
func (a *application) cancelWorkerRun() (proposalCommandResult, error) {
	...
	if a.state.Cancelled {
		...
		return proposalCommandResult{Status: "ok", Message: "supervise workflow is already durably cancelled"}, nil
	}
	run, found, err := a.readExactWorkerRun()
	if err != nil {
		return proposalCommandResult{}, err
	...
	return proposalCommandResult{Status: "ok", Message: "supervise workflow and its worker recurrence are durably cancelled"}, nil
}
